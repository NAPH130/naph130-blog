...
### 1.4 侧边栏组件按需渲染控制 (`showStats`) 与全站形态对齐
针对不同页面的信息密度与视觉统一性，`BlogSidebar` 提供 `showStats?: boolean` 开关属性：
- **文章页（`/posts`）与动态页（`/moments`）全面对齐**：统一传入 `showStats={false}` 取消多余的“站点统计”组件，两页面侧边栏形态严格一致，均仅保留“个人简介卡片”与“磨砂玻璃实时日历卡片”2 个核心侧边组件。
- **多语言数据动态注入**：两页面均统一采用 `getAllLocalePosts()` 计算 `statsByLocale` 并下发给 `BlogSidebar`，在切换语言时文章与标签数量实时联动更新。
- **空间与高度重新平衡**：移除中间统计卡片后，个人卡片分配 `flex-[0.9] min-h-[220px]`，日历卡片分配 `flex-[1.1] min-h-[260px]`，双卡片优雅舒展均分侧边高度，与右侧主面板底部严密齐平。
