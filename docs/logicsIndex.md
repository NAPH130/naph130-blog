...
| **13** | **动态星轨友邻与弹性碰撞** | [`logics/friendsPage.md`](./logics/friendsPage.md) | `src/content/friends` 多语言集合定义、二维完全弹性碰撞物理仿真引擎（Euler 积分、边框无损反弹、球体几何重叠分离与动量守恒冲量交换）、GPU 硬件加速 Direct DOM Transform 同步、悬停平滑锁止、自适应边界贴靠晶透探针卡片（Frosted HUD Inspector）与单层纯粹物理竞技场。 | 离散时间步长导致两球相交震颤粘滞锁死（重叠反向分离算法）、高频动画帧直接触发 React State 导致的重绘垃圾回收与掉帧（Ref 物理托管 + Direct Transform 调度）、悬停衰减后未恢复巡航速度导致的实体假死停止（恒定巡航速度自愈引擎）、球体碰撞外推导致刚体穿透边框切边（碰撞后二次边框严格钳制 + 44px 安全呼吸边界）。 |
